...
def search_flights(origin: str, destination: str) -> str:
    ...
    # Lấy dữ liệu từ DB (thử chiều xuôi, nếu không có thử chiều ngược)
    ...
    for f in flights:
        price_vnd = "{:,.0f}đ".format(f['price']).replace(",", ".")
        result += f"- {f['airline']} ({f['class']}): {f['departure']} -> {f['arrival']} | Giá: {price_vnd}\n"
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    # - Format đẹp. Nếu không có kết quả -> "Không tìm thấy khách sạn tại {city} với giá dưới {max_price_per_night}/đêm. Hãy thử tăng ngân sách."
    
    # 1. Tra cứu HOTELS_DB[city]
    # Sử dụng .get() để tránh lỗi nếu thành phố không có trong DB
    hotels = HOTELS_DB.get(city)
    
    if not hotels:
        return f"Xin lỗi, hiện tại tôi chưa có dữ liệu khách sạn tại thành phố '{city}'."

    # 2. Lọc theo max_price_per_night
    filtered_hotels = [h for h in hotels if h['price_per_night'] <= max_price_per_night]

    # 3. Nếu không có kết quả phù hợp sau khi lọc
    if not filtered_hotels:
        max_price_str = "{:,}đ".format(max_price_per_night).replace(",", ".")
    ...
    # sorted() với key là rating, reverse=True để lấy từ cao đến thấp
    sorted_hotels = sorted(filtered_hotels, key=lambda x: x['rating'], reverse=True)

    # 5. Format đẹp cho kết quả trả về
    result = f"Danh sách khách sạn tại {city} phù hợp với yêu cầu của bạn (đã sắp xếp theo đánh giá):\n"
    
    for h in sorted_hotels:
        # Định dạng giá tiền: 1.000.000đ
        price_str = "{:,}đ".format(h['price_per_night']).replace(",", ".")
        
        # Tạo chuỗi thông tin khách sạn
        result += (
            f"- {h['name']} ({h['stars']} sao)\n"
            f"  + Khu vực: {h['area']}\n"
            f"  + Giá: {price_str}/đêm\n"
            f"  + Đánh giá: {h['rating']}/5\n"
        )
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        total_expense = 0
        detail_lines = []
        # Tách các khoản bằng dấu phẩy, sau đó tách tên và giá bằng dấu hai chấm
        for item in expenses.split(','):
            name, price = item.split(':')
            price_val = int(price.strip())
            total_expense += price_val
            detail_lines.append(f"- {name.strip().capitalize()}: {price_val:,}đ".replace(",", "."))
            
        remaining = total_budget - total_expense
        
        # Tạo chuỗi phản hồi (sử dụng f-string và format tiền tệ)
        res = "Bảng chi phí:\n" + "\n".join(detail_lines) + "\n---\n"
        res += f"Tổng chi: {total_expense:,}đ\n".replace(",", ".")
        res += f"Ngân sách: {total_budget:,}đ\n".replace(",", ".")
        
        if remaining < 0:
            res += f"Vượt ngân sách {abs(remaining):,} đồng! Cần điều chỉnh.".replace(",", ".")
        else:
            res += f"Còn lại: {remaining:,}đ".replace(",", ".")
            
        return res
    except Exception:
        return "Lỗi: Định dạng chuỗi chi phí sai. Vui lòng dùng định dạng 'tên:số_tiền,tên:số_tiền'."
